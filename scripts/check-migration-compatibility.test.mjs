...
test("package, CI, and production deploy run the gate before migrations", async () => {
  ...
  assert.match(
    ...
    /^pnpm run check:migrations && pnpm run test:migration-compatibility && pnpm run test:reconciliation-stage && pnpm run test:production-backup-restore && pnpm run test:database-manifest && pnpm run supabase:reset && pnpm run test:migration-atomicity/,
  );
