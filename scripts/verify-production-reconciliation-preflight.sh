...
while (( $# > 0 )); do
  ...
  case "$option" in
    ...
    --passphrase-file) passphrase_file="$value" ;;
    --passphrase-sha256) passphrase_sha256="$value" ;;
...
for hash_and_label in \
  ...
  "$passphrase_sha256|--passphrase-sha256" \
...
if ! "$evidence_verifier" \
  ...
  --passphrase-sha256 "$passphrase_sha256" \
...
[[ "$line_number" == "18" ]] \
  || fail "evidence verifier emitted $line_number stdout lines; expected exactly 18."
...
printf '%s\n' \
  "PRODUCTION_RECONCILIATION_PREFLIGHT_SHA256=$preflight_sha256" \
  "$preflight_material"
