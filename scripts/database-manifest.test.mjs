...
test('platform-only baseline fixture excludes only the four exact later application Auth triggers', () => {
  ...
  const application = [
    ...
    'CREATE TRIGGER sync_site_admin_user_directory AFTER INSERT OR UPDATE OF email, created_at ON auth.users FOR EACH ROW EXECUTE FUNCTION private.sync_site_admin_user_directory();',
  ];
  const preserved = [
    'CREATE SCHEMA auth;',
    ...
    application[3].replace('private.sync_site_admin_user_directory()', 'auth.sync_site_admin_user_directory()'),
    application[3].replace('ON auth.users', 'ON auth.other_users'),
    application[3].replace('TRIGGER sync_site_admin_user_directory', 'TRIGGER unknown_directory_trigger'),
  ];
