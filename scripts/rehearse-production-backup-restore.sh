...
inspect_owned_container() {
  owned_snapshot="$($docker_bin container inspect "$container_name" --format \
    ...
    2>/dev/null)" || return 1
  expected_snapshot="$container_id|$postgres_image_id|$postgres_image|true|$capture_id|$restore_id|$ownership_token"
  [[ "$owned_snapshot" == "$expected_snapshot" ]]
}

adopt_attempted_container_for_cleanup() {
  attempted_snapshot="$($docker_bin container inspect "$container_name" --format \
    '{{.Id}}|{{.Image}}|{{.Config.Image}}|{{index .Config.Labels "com.dominion.production-backup-restore"}}|{{index .Config.Labels "com.dominion.capture-id"}}|{{index .Config.Labels "com.dominion.restore-id"}}|{{index .Config.Labels "com.dominion.ownership-token"}}' \
    2>/dev/null)" || return 1
  candidate_container_id="${attempted_snapshot%%|*}"
  candidate_snapshot_tail="${attempted_snapshot#*|}"
  expected_snapshot_tail="$postgres_image_id|$postgres_image|true|$capture_id|$restore_id|$ownership_token"
  [[ "$candidate_container_id" =~ ^[a-f0-9]{64}$ \
    && "$candidate_snapshot_tail" == "$expected_snapshot_tail" ]] || return 1
...
remove_owned_container() {
  ...
  "$docker_bin" rm --force "$container_name" >/dev/null || return 1
...
container_run_output="$($docker_bin run \
  ...
  "$postgres_image")"
adopt_attempted_container_for_cleanup || production_backup_fail \
  "new restore container failed the ownership and image inspection."
[[ "$container_run_output" == "$container_id" ]] || production_backup_fail \
...
actual_server_version="$($docker_bin exec "$container_name" \
