...
async function expectTargetClearOfCoachmark(page, targetId) {
  await expect(page.locator(`#${targetId}`)).toHaveClass(/crew-training-target/);
  await expect.poll(() => page.evaluate((id) => {
    const target = document.getElementById(id)?.getBoundingClientRect();
    const coachmark = document.getElementById('crewTrainingCoachmark')?.getBoundingClientRect();
    return Boolean(target && coachmark
      && (target.bottom <= coachmark.top || target.top >= coachmark.bottom));
  }, targetId), {
    message: `${targetId} should not be covered by the coachmark`,
  }).toBe(true);
}
...
test('safe-off training completes in the light theme without provider controls', async ({ page, app }) => {
  await page.route('**/src/static/group-integration-launch.mjs*', (route) => route.fulfill({
    status: 200,
    contentType: 'application/javascript',
    body: 'export function groupIntegrationsEnabled() { return false; }',
  }));
  await createCrew(page, app, 'Safe Off Crew', { theme: 'light' });
  const sensitiveBefore = await sensitiveCrewState(page);
  ...
  await page.locator('#crewTrainingNext').click();
  await expect(page.locator('#crewTrainingTitle')).toHaveText('Nothing to connect right now');
  await expect(page.locator('#crewTrainingCoachmark')).toHaveAttribute('aria-modal', 'true');
  await page.locator('#crewTrainingNext').click();
  await expect(page.locator('#crewTrainingProgress')).toHaveText('Step 7 of 7');
  await page.locator('#crewTrainingNext').click();
  await expect(page.locator('#crewTrainingLayer')).toBeHidden();
  expect(await trainingRows(page)).toMatchObject([{ status: 'completed' }]);
  expect(await sensitiveCrewState(page)).toEqual(sensitiveBefore);
});
...
test('contextual coachmarks remain onscreen on mobile', async ({ page, app }) => {
  ...
  await page.locator('#crewTrainingNext').click();
  await expect(page.locator('#crewTrainingProgress')).toHaveText('Step 2 of 7');
  await expectTargetClearOfCoachmark(page, 'copyInviteButton');
