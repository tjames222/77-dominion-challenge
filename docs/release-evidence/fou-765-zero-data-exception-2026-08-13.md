...
in the production-readiness release task on 2026-08-13. This exception applies
...
- Evidence captured: 2026-08-13T19:32Z
...
```sql
select
  (select count(*)::bigint from public.community_posts) as community_posts,
  ...
  (select count(distinct crew_id)::bigint
     from public.community_posts
    where crew_id is not null) as affected_groups,
  (select count(*)::bigint
