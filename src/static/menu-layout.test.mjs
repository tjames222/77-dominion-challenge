...
describe('shared sticky menu', () => {
  ...
  test('keeps the sticky layout footprint stable while compacting the visual surface', () => {
    ...
    const compactItems = declarationsFor('.topbar.topbar-collapsed > :not(.global-menu-button)');
    ...
    assert.doesNotMatch(compactItems, /(?:min-)?height\s*:|padding\s*:|margin\s*:/);
  });
  ...
  test('preserves mobile touch targets, safe areas, and reduced-motion preferences', () => {
    ...
    assert.doesNotMatch(declarationsFor('.topbar.topbar-collapsed > *'), /--topbar-item-scale/);
    assert.match(menuCss, />\s*:not\(\.global-menu-button\)\s*\{[^}]*--topbar-item-scale/s);
