...
type UserClient = { rpc: (name: string, args?: Record<string, unknown>) => Promise<RpcResult> };
type StorageObject = {
  size: number;
  type: string;
...
function safeDelivery(value: unknown) {
  ...
  const bucket = typeof delivery.bucket_name === "string" ? delivery.bucket_name : "";
  const path = typeof delivery.object_path === "string" ? delivery.object_path : "";
  const filename = typeof delivery.download_filename === "string" ? delivery.download_filename : "";
  const contentType = typeof delivery.content_type === "string" ? delivery.content_type : "";
  const sha256Hex = typeof delivery.sha256_hex === "string" ? delivery.sha256_hex : "";
  const sizeBytes = typeof delivery.size_bytes === "number" ? delivery.size_bytes : Number.NaN;
  if (
    bucket !== rewardBucket ||
...
function noStoreJson(body: unknown, status: number, req: Request, env: EnvReader) {
...
export function createHandler(overrides: Partial<Dependencies> = {}) {
  ...
  return async (req: Request) => {
    ...
    if (req.method !== "POST") return noStoreJson({ error: "Method not allowed." }, 405, req, dependencies.env);
    ...
    try {
      ...
      if (body.rewardKey !== rewardKey) throw new HttpError("The requested download is unavailable.", 400);
      ...
      if (requested.error) {
        if (requested.error.message?.includes("Too many download requests")) {
          throw new HttpError("Too many download requests. Try again shortly.", 429);
        }
      ...
      const delivery = safeDelivery(Array.isArray(redeemed.data) ? redeemed.data[0] : null);
      if (redeemed.error || !delivery) throw new HttpError("The download request expired. Try again.", 410);

      const storage = admin.storage.from(delivery.bucket);
      const downloaded = await storage.download(delivery.path);
      const bytes = downloaded.error ? null : await verifiedApprovedAsset(downloaded.data, delivery);
      if (!bytes) {
        throw new HttpError("The approved download failed its integrity check.", 410);
      }
      ...
      return new Response(bytes, {
        ...
        headers: {
          ...
          "Access-Control-Expose-Headers": "Content-Disposition, X-Reward-Filename",
      ...
      if (ticket && userId) {
        try {
          await dependencies.createAdminClient().rpc("record_reward_download_result", {
            target_token: ticket,
            target_user_id: userId,
      ...
      dependencies.logger.error("Reward download failed", error instanceof HttpError ? error.message : "internal");
      const response = errorResponse(error, "The handbook download is temporarily unavailable.", req, dependencies.env);
      response.headers.set("Cache-Control", "private, no-store, max-age=0");
      response.headers.set("X-Content-Type-Options", "nosniff");
      response.headers.set("Referrer-Policy", "no-referrer");
      return response;
