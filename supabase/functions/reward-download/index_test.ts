import { assertEquals, quietLogger, responseJson } from "../_shared/test_helpers.ts";
...
const validPdfChecksum = "025bfabf088fa4396e6638c23e49688c077386554ebe472d6f941d8974bda128";
const nonPdfChecksum = "3ef38a2f3b4986776f699401812197c045833fa192240b701b9915fb68ed9e28";
  ...
  const user = {
    rpc: async (name: string) => {
      ...
      return options.unavailable
        ...
        : { data: { availability: "available", ticket: "10000000-0000-4000-8000-000000000001" }, error: null };
    },
  };
  const admin = {
    rpc: async (name: string) => {
      ...
      if (name === "redeem_reward_download_ticket") {
        return { data: [{
          ...
          content_type: options.unsafeDelivery ? "text/html" : "application/pdf",
          sha256_hex: options.assetState === "not-pdf" ? nonPdfChecksum : validPdfChecksum,
          size_bytes: 22,
        }], error: null };
      }
...
Deno.test("reward download streams the exact verified bytes without exposing storage coordinates", async () => {
  ...
  assertEquals(response.headers.get("content-disposition"), 'attachment; filename="Nehemiah-Leadership-Handbook.pdf"');
  ...
  assertEquals(response.headers.get("content-security-policy"), "default-src 'none'; sandbox");
  assertEquals(await response.text(), validPdf);
  assertEquals(test.fixture.calls.includes("storage.download"), true);
  assertEquals(test.fixture.calls.includes("record_reward_download_result"), true);
  assertEquals(JSON.stringify([...response.headers]).includes("handbooks/nehemiah-v1.pdf"), false);
});
...
Deno.test("missing approved asset stays unavailable without a placeholder", async () => {
  ...
  assertEquals(test.fixture.calls.includes("redeem_reward_download_ticket"), false);
});

Deno.test("unsupported rewards and stale actors fail closed", async () => {
  const unsupported = handler();
  assertEquals((await unsupported.handle(request({ rewardKey: "other", expectedUserId: "user-1" }))).status, 400);

  const staleActor = handler();
  assertEquals((await staleActor.handle(request({
    rewardKey: "nehemiah_leadership_handbook",
