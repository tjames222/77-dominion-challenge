...
-- 01-10: the production program contract is singular, current, Solo-only,
...
), 1, 'the Solo first-run program has one immutable version-1 definition');
...
), 13, 'the Solo first-run program publishes all thirteen reviewed pages');
...
select is((
  ...
  'dashboard,bible-reading,morning-prayer,worship,evening-prayer,workout-one,intentional-walk,workout-two,badges-rewards,community,profile,billing,science',
...
select is((
  ...
  '/dashboard.html,/bible-reading.html,/morning-prayer.html,/worship.html,/evening-prayer.html,/workout-one.html,/intentional-walk.html,/workout-two.html,/badges-rewards.html,/community.html,/profile.html,/billing.html,/science.html',
...
select ok(not exists (
  ...
  where page.program_id = 'solo-first-run'
    and page.program_version = 1
    and (page.page_content_version <> 1 or not definition.is_current)
), 'every program page pins current content version 1');
...
select is((
  ...
  where definition.page_id in (
    ...
    'badges-rewards', 'community', 'profile', 'billing', 'science'
  ) and definition.content_version = 1
), 13, 'all thirteen versioned page definitions were published');
...
-- 11-23: each page publishes the exact stable step IDs mirrored by the client
...
select is((select step_ids from private.site_training_page_versions
  where page_id = 'dashboard' and content_version = 1),
...
select is((select step_ids from private.site_training_page_versions
  where page_id = 'community' and content_version = 1),
...
-- 24-27: definitions stay behind the authenticated RPC boundary, completion
...
-- 28-34: Start has exact replay semantics, CAS rejects stale tabs, and Stop /
...
select throws_ok(
  $$select public.transition_site_training(
    'overall', 'dashboard', 1, 'solo-first-run', 1, 'stop',
...
values
  ...
  ('resume', public.claim_site_training(
    'overall', 'dashboard', 1, 'solo-first-run', 1, 'resume',
...
-- 35-40: ordered movement reaches exactly the reviewed final Dashboard step;
...
values
  ('finish', public.transition_site_training(
    'overall', 'dashboard', 1, 'solo-first-run', 1, 'finish',
...
select is((
  ...
  where completion.user_id = 'f1000000-0000-4000-8000-000000000001'
    ...
    and completion.content_version = 1
...
-- 41-48: another actor sees a clean state, account deletion cascades only that
...
values ('isolated-start', public.claim_site_training(
  'page', 'dashboard', 1, null, null, 'start',
...
select is((
  ...
  where progress.page_id = 'dashboard'
    and progress.content_version = 1
